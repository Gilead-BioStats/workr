...
support configurable `LoadData` hooks. You can supply either a function
or the name of a registered provider through `lConfig$LoadData`.

{workr} includes a built-in `"gsm.datasim"` provider for generating
workflow inputs from the optional
...
The provider reads its options from `lConfig$gsm.datasim`. Common fields
