...
The workflow should reference the objects generated by {gsm.datasim}.
For example, this YAML summarizes enrolled subjects by simulated site
from `Raw_SUBJ`:
...
Then configure
[`RunWorkflow()`](https://gilead-biostats.github.io/workr/dev/reference/RunWorkflow.md)
to load the simulated study data before the first step runs:
...
include `profile`, `study_type`, `participants`, `sites`,
`snapshot_count`, `months_duration`, and `snapshot`. Generated `Raw_*`
objects, such as `Raw_SUBJ` and `Raw_AE`, are also exposed as `df*`
