...
::: {.notes}
This is the real bundled example (inst/example_workflows/01_RunWorkflow).
Reveal 1: the whole file. Reveal 2: `meta` is metadata. Reveal 3: `steps` is an
ordered list of function calls; each names a function, an `output`, and its
`params`. `lData` accumulates the outputs. That's the entire model.
:::
...
Keep this brief — it's the contract.
:::
...
`workr::DemoApp_init()` · live at
[jwildfire.shinyapps.io/workr-demoapp](https://jwildfire.shinyapps.io/workr-demoapp/)
:::

::: {.notes}
If live: change `col` from `speed` to `dist` — the result updates and the steps
stay put. The logic is data you can edit, diff, and review rather than a script
you rewrite. Point: declarative definitions are easier for others to trust and
audit.
:::
...
[{qcthat}](https://github.com/Gilead-BioStats/qcthat) links requirements to tests — qualify one framework, not 27,000 scripts.
:::
...
::: {.notes}
Ties back to the 27k-metrics story: orchestration and hooks make that scale
claim work in a regulated pipeline.
:::
