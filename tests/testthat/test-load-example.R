test_that("loadExample returns input data when initData.R is missing (#9) and (#26)", {
...
test_that("loadExample loads initData.R and merges with lData (#9) and (#26)", {
...
test_that("loadExample validates initData return type (#9) and (#26)", {
  td <- tempfile("workflow_dir_")
  dir.create(td, recursive = TRUE)
...
test_that("loadExample does not create duplicate names on repeated calls (#9) and (#26)", {
  td <- tempfile("workflow_dir_")
  dir.create(td, recursive = TRUE)
